...
class SM64_AnimHeader:
    ...
    def to_binary(
        ...
        if segment_data:
            ...
            data.extend(values_address.to_bytes(4, byteorder="big", signed=False))  # 0x0C
            data.extend(indice_address.to_bytes(4, byteorder="big", signed=False))  # 0x10
        ...
        for i, element in enumerate(self.elements):
            if element.header:
                ...
                if segment_data:
                    ...
                    table_data.extend(header_offset.to_bytes(4, byteorder="big", signed=False))
            else:
                assert isinstance(element.reference, int), f"Reference at element {i} is not an int."
                table_data.extend(element.reference.to_bytes(4, byteorder="big", signed=False))
