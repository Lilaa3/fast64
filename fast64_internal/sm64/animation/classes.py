...
class SM64_AnimTable:
    ...
    def prepare_for_dma(self):
        ...
        for i, anim_header in enumerate(self.elements):
            ...
            if (i < len(self.elements) - 1) and self.elements[i + 1].data == anim_header.data:
                continue
            ...
            for included_header in included_headers:
                ...
                elements.append(included_header)
...
def create_tables(anims_data: list[SM64_AnimData], values_name: str = None):
    ...
    for pair in all_pairs:
        values = pair.values
        max_frame = len(values)
        if max_frame > MAX_U16:
            raise PluginError("Pair frame count is higher than the 16 bit max")
    ...
    for value_table_part_pairs, value_table_part in value_table_parts:
        for pair in value_table_part_pairs:
            ...
            if pair.offset > MAX_U16:
                raise PluginError("Pair offset is higher than the 16 bit max.")
